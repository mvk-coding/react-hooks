...
> some notes:
> - useState and useReducer can be both useful, but it is important to evaluate
>  the use case and decide which one to use.
> - the order in which states are updates matters if you have to update multiple 
> states in a single function; an update to a state can trigger a re-render and
> if updates states are dependent on each other, it can cause unexpected behavior.
## Background
