...
> Lifting states means that shared states are moved to the lowest common
> parent and passing the states and handlers to the relevant children.
> Colocating is also important, which is the opposite process to move a state
> that is no longer shared to the component that needs it.
