...
> Knowing why to use useEffect, namely to add side effects to certain events,
> is important. I was alsno not aware how I could get access to the values in 
> the local storage in DevTools, it is good to know there is a tab for 
> that information too.
> The lazy state initialization is a good way to avoid unnecessary rerendering,
> it is useful to know you can enter a function instead of a variable to achieve 
> this.
> Furthermore, I was not aware that the array as a second argument to useEffect 
> was optional. I wasn't aware either that those dependencies are used to avoid 
> unnecessary rerendering too. 
> It is fun to build a custom hook to generalize behaviour using other hook 
> functions. Important note to self; start the naming of custom hooks with 'use'.
